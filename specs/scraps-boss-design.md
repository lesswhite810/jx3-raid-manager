...
### 3.5 数量统计（v2.2.1 修正：数量而非次数）

`count` 语义为**购买数量**，不是交易次数。解析流程：

1. **种类识别**：每条 `[买家]花费[金额]购买了[物品名]` 消息确定一次交易（与此前一致）；
2. **数量配对**：取该消息**前后 2 秒内**同物品的本地入包日志 `你获得：[物品名] × N。`
   （MSG_ITEM 类型，无 `×N` 后缀按 1 计），多条到账累加，每条到账只被消费一次；
3. **回退**：窗口内找不到到账事件时数量按 1 笔计（到账日志缺失场景）；
4. **回显抑制**：无独立到账事件、且与已接受交易同物品+同金额+间隔 ≤3 秒的消息，
   视为 MSG_ROOM/MSG_WHISPER 双频道对同一笔交易的重复播报，整条丢弃
   （不计数量、不累计金额、不加 drops）。

> 背景：散件结算时团长批量发放材料（每笔可给多个，如"你获得：[玛瑙] × 17。"），
> 且同一物品常连续多笔 0 金交易。旧版按消息条数计数 + ±30 秒时间桶去重，
> 会把连击交易误判为回显（茶饼 5 笔记成 1），也无法体现单笔多数量。

示例（对应真实副本流水）：

```
23:43:00 购买了[肆级五彩石](5000金) ←→ 你获得:[肆级五彩石]        → +1
23:42:24 购买了[玛瑙](0金)       ←→ 你获得:[玛瑙] × 17           → +17
23:42:29 购买了[肆级五彩石](0金) ←→ 你获得:[肆级五彩石]×1 + ×1    → +2
```
...
#[derive(Debug, Clone)]
struct PurchaseInfo {
    count: u32,         // 购买数量：购买消息 ±2s 内"你获得 ×N"配对求和，缺失回退 1
...
- 散件清单计算：
  ...
  - 非 0 金装备不计入散件
  - 同物品多次购买 count 累加正确
  - 购买数量配对：±2 秒内 `你获得 ×N` 求和、窗口边界、多条到账累加
  - 回显抑制：无到账事件 + 同物品同金额 ≤3 秒的重复消息丢弃
  - 有独立到账的近间隔两笔真实交易均计数
  - 无到账日志回退数量 1；他人购买与他人"XX获得："不参与
