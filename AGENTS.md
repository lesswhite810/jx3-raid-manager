...
- 功能、Bug、版本变更时，同步更新 `README.md` 的「📋 更新日志」；该处**仅保留最近 5 个版本**，更早的版本块删除，完整历史以 `release-notes/` 与 GitHub Releases 为准。
...
- `npm run tauri build`：本地默认只生成可执行文件，不打安装包；同时会启用快速本地 release 配置（incremental、较高 `codegen-units`、`rust-lld`、`lto=false`、`strip=false`）以缩短反复构建时间。生产构建（CI / `JX3_TAURI_FULL_LOCAL_BUILD=1`）会使用 `Cargo.toml` 中 `[profile.release]` 的 `lto="fat"` + `strip="symbols"` + `codegen-units=1` 完整优化。
...
- GitHub Release 工作流使用 `gh release create` / `gh release upload` 上传资产，避免依赖 Node 20 的第三方 action。
- GitHub Release 工作流启用 `Swatinem/rust-cache@v2` 缓存 Rust 依赖，加速构建。
- CI 工作流（`.github/workflows/ci.yml`）在 PR 和 push 到 master 时运行前端 `npm build` + `npm test` 和 Rust `cargo check --release` + `cargo test --release` 作为质量门禁。
