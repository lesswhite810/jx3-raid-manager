// 一次性脚本：用 Node 重写 tailwind.config.js,UTF-8,保留 CRLF 行尾(若原文件是 CRLF)
// 注入设计系统提案 token(aliases,ds- 前缀)
import fs from 'node:fs';
import path from 'node:path';

const filePath = path.resolve('tailwind.config.js');
const original = fs.readFileSync(filePath, 'utf8');
const EOL = original.includes('\r\n') ? '\r\n' : '\n';

const NEW = `/** @type {import('tailwindcss').Config} */
export default {
  content: [
    "./index.html",
    "./*.{js,ts,jsx,tsx}",
...
console.log(`✓ tailwind.config.js 已重写（${NEW.length} 字节 · EOL = ${EOL === '\r\n' ? 'CRLF' : 'LF'}）`);
