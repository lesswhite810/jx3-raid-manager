...
///
/// 返回 (item_name -> price) 的映射，仅包含已缓存的物品。
/// 调用方对未命中的物品保持 unit_price = None（前端手填）。
fn query_prices_by_names(conn: &Connection, names: &[String]) -> HashMap<String, i64> {
    let mut result = HashMap::new();
    for name in names {
        if let Ok(price) = conn.query_row(
            "SELECT price FROM drop_items WHERE item_name = ?1 AND price IS NOT NULL",
            params![name],
            |row| row.get::<_, i64>(0),
        ) {
...
fn compute_scraps_items(
    ...
    for (name, info) in purchased_items {
        let category = item_categories.get(name).map(|s| s.as_str()).unwrap_or("");
        let is_material = SCRAPS_MATERIAL_WHITELIST.contains(&name.as_str());
        let is_zero_price_equipment = category == "equipment" && info.total_price == 0;

        if !is_material && !is_zero_price_equipment {
            continue;
        }
        ...
        if let Some(p) = unit_price {
            total_value += info.count as i64 * p;
        }

        items.push(ScrapsItem {
            name: name.clone(),
            count: info.count,
            unit_price,
...
/// purchased_items 为当前角色购买的物品映射（物品名 → PurchaseInfo），用于散件老板功能。
...
fn extract_drops_from_chatlog(
    ...
    end_time_ms: i64,
    role_name: &str,
) -> Result<(Vec<String>, Option<i64>, i64, i64, u32, Vec<(i64, i64)>, Vec<i64>, HashMap<String, PurchaseInfo>), String> {
    if !chatlog_path.exists() {
        return Ok((Vec::new(), None, 0, 0, 0, Vec::new(), Vec::new(), HashMap::new()));
    }
...
///
/// 返回 (drops, base_salary, other_income_gold, expense_gold, boss_kill_count,
///       income_records, boss_kill_times, purchased_items)。
/// purchased_items 为当前角色购买的物品映射（物品名 → PurchaseInfo），用于散件老板功能。
fn extract_drops_from_records(
    ...
    end_time_ms: i64,
    role_name: &str,
) -> (Vec<String>, Option<i64>, i64, i64, u32, Vec<(i64, i64)>, Vec<i64>, HashMap<String, PurchaseInfo>) {
    let start_sec = start_time_ms / 1000;
    let end_sec = end_time_ms / 1000;
    ...
    // 购买物品总件数（含重复购买）
    let purchased_total_count: u32 = purchased_items_map.values().map(|i| i.count).sum();

    log::info!(
        "[DropScanner] chatlog 提取完成: {} 个物品分配, {} 条金币获得, 底薪={:?}, 其他收入 {} 金, 支出 {} 金, BOSS击杀奖励 {} 次, 购买物品 {} 种 {} 件",
        item_count,
        money_count,
        base_salary,
        other_income_gold,
        expense_gold,
        boss_kill_count,
        purchased_items_map.len(),
        purchased_total_count
    );

    (drops, base_salary, other_income_gold, expense_gold, boss_kill_count, income_records, boss_kill_times, purchased_items_map)
}
...
fn upsert_raid_drop_record(
    ...
    for name in purchased_items.keys() {
    ...
    let record = serde_json::json!({
        ...
        "scrapsItems": scraps_items,
        "scrapsValue": scraps_value,
        "isScrapsBoss": false,
        "notes": notes,
    });
...
pub fn scan_raid_drops_with_raids(
    ...
    for instance in instances.iter_mut() {
        ...
        // 合并所有 chatlog 文件的购买物品映射（物品名 → PurchaseInfo，用于散件老板功能）
        let mut all_purchased_items: HashMap<String, PurchaseInfo> = HashMap::new();
        ...
        // 合并购买物品映射：同物品累加 count 和 total_price
        for (name, info) in purchased_items {
            let entry = all_purchased_items.entry(name).or_default();
            entry.count += info.count;
            entry.total_price += info.total_price;
        }
        if salary.is_some() {
            base_salary = salary;
        }
...
/// 散件记录购买价回填结果（前端使用 camelCase 字段）
#[derive(Debug, Clone, serde::Serialize)]
#[serde(rename_all = "camelCase")]
pub struct ScrapsBackfillResult {
    /// 扫描到的待回填散件记录数（isScrapsBoss && 无 totalPrice）
    pub scanned: usize,
    /// 成功回填并写入 DB 的记录数
    pub updated: usize,
    /// 跳过的记录数（chatlog 已不可用 / 无购买消息匹配 / 数据异常等）
    pub skipped: usize,
    /// 处理失败的记录 ID 列表
...
/// 从 JCL 文件名解析副本首段（`YYYY-MM-DD-HH-MM-SS`）对应的本地时间戳（毫秒）
///
/// 文件名格式：`YYYY-MM-DD-HH-MM-SS-副本名(mapid)-BOSS(...).jcl`
fn parse_jcl_filename_timestamp_ms(file_name: &str) -> Option<i64> {
    if file_name.len() < 19 {
        return None;
    }
    let head = &file_name[..19];
    let parts_split: Vec<&str> = head.split('-').collect();
    if parts_split.len() != 6 {
        return None;
    }
    let y = parts_split[0].parse::<i32>().ok()?;
    let mo = parts_split[1].parse::<u32>().ok()?;
    let d = parts_split[2].parse::<u32>().ok()?;
    let h = parts_split[3].parse::<u32>().ok()?;
    let mi = parts_split[4].parse::<u32>().ok()?;
    let s = parts_split[5].parse::<u32>().ok()?;
...
/// 回填历史散件记录中每个物品的实际购买价（totalPrice）
///
/// 适用对象：isScrapsBoss=true 且 scrapsItems 中没有任何物品携带 totalPrice 的旧版记录。
///
/// 处理流程：
/// 1. 读取 app DB 中所有 records，筛选出待回填记录
/// 2. 枚举 game_dir/interface/my#data/ 下的茗伊账号目录，解析 info.jx3dat
///    建立 (角色名, 服务器) -> 聊天记录文件列表的映射（茗伊账号目录名是数字 uid，
///    与 records.accountId UUID 格式不同，必须通过 info.jx3dat 反查）
/// 3. 对每条记录，从 JCL 文件名解析副本时间窗口；无 JCL 文件时退回到 record.date ± 缓冲
/// 4. 用 preload_chatlog_records 预加载窗口内的 chatlog 记录，
///    然后用 extract_drops_from_records 提取购买映射（物品 -> PurchaseInfo）
/// 5. 对 scrapsItems 中每个物品：
///    - category=equipment -> totalPrice=0（散件装备来源是 0 金购买，免费获取）
///    - 其他（白名单材料）-> 同名 PurchaseInfo.total_price；未匹配到则 0
/// 6. 至少匹配到 1 条购买消息或装备时，写回 DB；否则跳过避免误置 0
pub fn backfill_scraps_purchase_prices_internal() -> Result<ScrapsBackfillResult, String> {
    ...
    let mut stmt = conn
        ...
        .map_err(|e| format!("准备查询记录失败: {}", e))?;
    let mut rows = stmt.query([]).map_err(|e| format!("查询记录失败: {}", e))?;
    ...
    let mut legacies: Vec<LegacyRecord> = Vec::new();
    while let Some(row) = rows.next().map_err(|e| format!("遍历记录失败: {}", e))? {
        let id: String = match row.get(0) {
            Ok(s) => s,
            Err(_) => continue,
        };
        let data_str: String = match row.get(1) {
            Ok(s) => s,
            Err(_) => continue,
        };
        ...
        let is_scraps = parsed.get("isScrapsBoss").and_then(|v| v.as_bool()).unwrap_or(false);
        if !is_scraps {
            continue;
        }
        let items = parsed.get("scrapsItems").and_then(|v| v.as_array()).cloned().unwrap_or_default();
        if items.is_empty() {
            continue;
        }
        let has_total = items.iter().any(|it| it.get("totalPrice").and_then(|v| v.as_i64()).is_some());
        if has_total {
            continue;
        }
        let role_name = parsed.get("roleName").and_then(|v| v.as_str()).unwrap_or("").to_string();
        let server = parsed.get("server").and_then(|v| v.as_str()).unwrap_or("").to_string();
        let date_ms = parsed.get("date").and_then(|v| v.as_i64()).unwrap_or(0);
        let jcl_files: Vec<String> = parsed.get("jclFiles").and_then(|v| v.as_array()).map(|arr| {
            arr.iter().filter_map(|x| x.as_str().map(|s| s.to_string())).collect()
        }).unwrap_or_default();
        legacies.push(LegacyRecord { id, data: parsed, role_name, server, date_ms, jcl_files });
    }
    drop(rows);
    drop(stmt);

    let total = legacies.len();
    if total == 0 {
        return Ok(ScrapsBackfillResult { scanned: 0, updated: 0, skipped: 0, failed_ids: Vec::new(), message: "无需回填".to_string() });
    }

    let game_dir = match get_game_directory() {
        Ok(d) => d,
        Err(e) => {
            return Ok(ScrapsBackfillResult {
                scanned: total,
                updated: 0,
                skipped: total,
                failed_ids: legacies.iter().map(|r| r.id.clone()).collect(),
                message: format!("未配置游戏目录，跳过: {}", e),
            });
        }
    };
    let game_path = PathBuf::from(&game_dir);
    ...
    // 枚举 my#data/*@zhcn_hd 目录，解析 info.jx3dat 建立 (角色名, 服务器) -> 聊天记录文件列表
    // 注意：茗伊账号目录名是**数字 uid**（与 records.accountId UUID 不同），
    // 必须通过 info.jx3dat 反查角色身份才能关联到对应账号。
    let mut role_to_chatlogs: HashMap<(String, String), Vec<PathBuf>> = HashMap::new();
    if let Ok(entries) = std::fs::read_dir(&accounts_base) {
        for entry in entries.flatten() {
            ...
            let info_path = entry.path().join("info.jx3dat");
            if !info_path.exists() {
                continue;
            }
            let identity = match crate::mingyi::active_detector::parse_info_jx3dat(&info_path, Some(&game_path)) {
                Some(id) => id,
                None => continue,
            };
            let role_name_norm = identity.role_name.replace(' ', "").replace('　', "");
            let chatlog_files = find_chatlog_files(&entry.path()).unwrap_or_default();
            if chatlog_files.is_empty() {
                continue;
            }
            role_to_chatlogs.insert(
                (role_name_norm, identity.server.clone()),
                chatlog_files,
            );
        }
    }
    ...
    for legacy in &legacies {
        let (win_start, win_end) = if !legacy.jcl_files.is_empty() {
            let mut min_ts: Option<i64> = None;
            let mut max_ts: Option<i64> = None;
            for f in &legacy.jcl_files {
                if let Some(ts) = parse_jcl_filename_timestamp_ms(f) {
                    min_ts = Some(min_ts.map_or(ts, |m| m.min(ts)));
                    max_ts = Some(max_ts.map_or(ts, |m| m.max(ts)));
                }
            }
            if let (Some(lo), Some(hi)) = (min_ts, max_ts) {
                (lo - 5 * 60 * 1000, hi + 2 * 60 * 60 * 1000)
            } else {
                (legacy.date_ms - 15 * 60 * 1000, legacy.date_ms + 4 * 60 * 60 * 1000)
            }
        } else {
            (legacy.date_ms - 15 * 60 * 1000, legacy.date_ms + 4 * 60 * 60 * 1000)
        };
        ...
        let lookup_key = (
            legacy.role_name.replace(' ', "").replace('　', ""),
            legacy.server.clone(),
        );
        let chatlog_files = match role_to_chatlogs.get(&lookup_key) {
            Some(v) if !v.is_empty() => v.clone(),
            _ => { skipped += 1; continue; }
        };

        let range_cache: HashMap<PathBuf, Option<(i64, i64)>> = chatlog_files.iter().map(|p| (p.clone(), get_chatlog_time_range(p))).collect();
        let preloaded = preload_chatlog_records(&chatlog_files, &range_cache, win_start, win_end);
        let (_, _, _, _, _, _, _, purchased_items) = extract_drops_from_records(&preloaded, win_start, win_end, &legacy.role_name);
        ...
        let items = legacy.data.get("scrapsItems").and_then(|v| v.as_array()).cloned().unwrap_or_default();
        ...
        for item in &items {
            ...
            if category == "equipment" {
                ni["totalPrice"] = serde_json::json!(0);
                matched += 1;
            } else if !name.is_empty() {
                let tp = purchased_items.get(name).map(|p| p.total_price).unwrap_or(0);
                ni["totalPrice"] = serde_json::json!(tp);
                if purchased_items.contains_key(name) {
                    matched += 1;
                }
            }
            new_items.push(ni);
        ...
        let mut new_data = legacy.data.clone();
        new_data["scrapsItems"] = serde_json::Value::Array(new_items);
        let new_data_str = match serde_json::to_string(&new_data) {
            Ok(s) => s,
            Err(_) => { failed_ids.push(legacy.id.clone()); continue; }
        };
        match conn.execute(
            "UPDATE records SET data = ?1, updated_at = ?2 WHERE id = ?3",
            params![new_data_str, now_ms, &legacy.id],
        ) {
            Ok(_) => updated += 1,
            Err(e) => {
                log::warn!("[Backfill] 更新记录 {} 失败: {}", legacy.id, e);
                failed_ids.push(legacy.id.clone());
            }
        }
    }

    let message = if updated == 0 {
        format!("扫描 {} 条历史散件记录，无需更新（chatlog 数据可能已不可用或未匹配到购买消息）", total)
    } else {
        format!("已回填 {} 条历史散件记录的实际购买价", updated)
    };
    log::info!("[Backfill] 散件购买价回填: scanned={} updated={} skipped={} failed={}", total, updated, skipped, failed_ids.len());

    Ok(ScrapsBackfillResult { scanned: total, updated, skipped, failed_ids, message })
}

/// 前端调用的 tauri 命令入口
#[tauri::command]
pub async fn backfill_scraps_purchase_prices() -> Result<ScrapsBackfillResult, String> {
    tokio::task::spawn_blocking(backfill_scraps_purchase_prices_internal)
        .await
        .map_err(|e| format!("回填线程异常: {}", e))?
}
...
mod tests {
    ...
    /// 测试辅助：确保 drop_items 表存在（init_db_with_path 当前未创建该表）
    fn ensure_drop_items_table_for_test(conn: &rusqlite::Connection) {
        conn.execute_batch(
            r#"
        ...
        .expect("should ensure drop_items table");
    }

    /// 插入测试用 drop_items 记录
    fn insert_test_drop_item(
        conn: &rusqlite::Connection,
        name: &str,
        price: Option<i64>,
        category: &str,
    ) {
        ensure_drop_items_table_for_test(conn);
        let now = chrono::Local::now().to_rfc3339();
        conn.execute(
            "INSERT INTO drop_items (item_type, item_id, item_name, price, category, created_at)
             VALUES (5, 0, ?1, ?2, ?3, ?4)",
            sql_params![name, price, category, now],
        )
        .expect("should insert drop_item");
    }

    /// 构造购买记录
    fn make_purchase(count: u32, total_price: i64) -> PurchaseInfo {
        PurchaseInfo { count, total_price }
    }

    /// 测试：空购买记录应返回空散件清单
    #[test]
    fn test_compute_scraps_items_empty() {
        let (conn, _temp_dir) = create_scan_test_db();
    ...
    /// 测试：白名单材料非 0 金购买也计入（按设计 §1.2：0 金或非 0 金都算）
    #[test]
    fn test_compute_scraps_items_whitelist_material_nonzero_price() {
        let (conn, _temp_dir) = create_scan_test_db();
        let mut purchased = HashMap::new();
    ...
    #[test]
    fn test_compute_scraps_items_zero_price_equipment_no_price() {
        let (conn, _temp_dir) = create_scan_test_db();
    ...
    /// 测试：非 0 金装备不计入散件
    #[test]
    fn test_compute_scraps_items_nonzero_equipment_excluded() {
        let (conn, _temp_dir) = create_scan_test_db();
        insert_test_drop_item(&conn, "竞拍装备", Some(50000), "equipment");
        let mut purchased = HashMap::new();
    ...
    /// 测试：不在白名单的材料不计入散件
    #[test]
    fn test_compute_scraps_items_non_whitelist_material_excluded() {
        let (conn, _temp_dir) = create_scan_test_db();
        let mut purchased = HashMap::new();
        // 副本外的杂物材料（不在白名单）
    ...
    /// 测试：0 金装备但 category 非 equipment 不计入
    #[test]
    fn test_compute_scraps_items_zero_price_non_equipment_excluded() {
        let (conn, _temp_dir) = create_scan_test_db();
        insert_test_drop_item(&conn, "神秘物品", Some(1000), "unknown");
        let mut purchased = HashMap::new();
    ...
    /// 测试：0 金装备但 drop_items 无 category 记录 → 保守策略不计入（设计 §8.4）
    #[test]
    fn test_compute_scraps_items_zero_price_no_category_excluded() {
        let (conn, _temp_dir) = create_scan_test_db();
        // 不插入 drop_items 记录，category 缺失
        let mut purchased = HashMap::new();
    ...
    /// 测试：同物品多次购买 count 累加正确
    #[test]
    fn test_compute_scraps_items_count_accumulation() {
        let (conn, _temp_dir) = create_scan_test_db();
        let mut purchased = HashMap::new();
    ...
    /// 测试：混合场景（白名单材料 + 0 金装备 + 非 0 金装备 + 非白名单材料）
    /// 验证 scrapsValue 求和正确性（含 unitPrice=None 按 0 计入）
    #[test]
    ...
    /// 测试：从 JCL 文件名解析本地时间戳（用于散件回填窗口计算）
    #[test]
    fn test_parse_jcl_filename_timestamp_ms() {
        // 正常文件名前缀
        let ms = parse_jcl_filename_timestamp_ms(
            "2026-08-24-22-56-42-25人英雄阆风悬城(795)-xxxxx.jcl",
        );
        assert!(ms.is_some(), "应能解析合法文件名");
        // 已知时间：2026-08-24 22:56:42 local -> 与 record.date 一致即 1787583102000 附近
        let v = ms.unwrap();
        // 与秒级基准校验（误差需在 1 秒内）
        let expected_sec = 1787583402_i64;
        assert!(
            (v / 1000 - expected_sec).abs() <= 1,
            "解析结果与基准时间不符: {} vs {}",
            v / 1000,
            expected_sec
        );
        ...
        // 长度不足返回 None
        assert!(parse_jcl_filename_timestamp_ms("abc.jcl").is_none());
        // 非时间前缀返回 None
        assert!(parse_jcl_filename_timestamp_ms("not-a-jcl-file.jcl").is_none());
    }

    /// 测试：输出按名称排序（UTF-8 字节序）
    #[test]
    fn test_compute_scraps_items_sorted_by_name() {
        let (conn, _temp_dir) = create_scan_test_db();
        let mut purchased = HashMap::new();
        // 故意以乱序插入
        purchased.insert("猫眼石".to_string(), make_purchase(1, 0));
        ...
        let names: Vec<&str> = items.iter().map(|i| i.name.as_str()).collect();
        assert_eq!(names, vec!["猫眼石", "玛瑙", "维峰丹"], "应按名称排序");
    }
