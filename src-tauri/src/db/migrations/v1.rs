...
///
/// 前置条件：无（即使旧 accounts 表不存在，也会创建新的结构化表）
///
/// 迁移内容：
/// - 创建 accounts 结构化表
/// - 创建 roles 结构化表
/// - 如存在旧 accounts 表（JSON blob 格式），将数据迁移到新表
...
pub fn migrate(conn: &Connection) -> Result<(), String> {
    ...
    let has_data_column: i64 = if has_old_table > 0 {
        conn.query_row(
    ...
    let needs_migration = has_old_table > 0 && has_data_column > 0;
    ...
    if needs_migration {
        ...
        log::info!("V1 迁移：不存在旧 accounts 表或已是新格式，仅创建结构化表");
